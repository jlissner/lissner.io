...
export const EmojiPicker = ({ onEmojiSelect, onClose }: EmojiPickerProps) => {
  ...
  return (
    ...
    >
      ...
      <div data-grid="8" style={{ marginBottom: '0.75rem', maxHeight: '8rem', overflowY: 'auto' }}>
        {filteredEmojis.map((item) => (
          <button
            key={item.emoji}
            onClick={() => handleEmojiClick(item.emoji)}
            style={{ width: '2rem', height: '2rem', fontSize: '1.125rem', display: 'flex', alignItems: 'center', justifyContent: 'center', borderRadius: '0.25rem', transition: 'background-color 0.2s', cursor: 'pointer' }}
      ...
      {searchTerm.trim() && filteredEmojis.length === 0 && (
        <div style={{ textAlign: 'center', padding: '1rem 0', color: 'var(--neutral-500)', fontSize: '0.875rem' }}>
          <p>No emojis found for "{searchTerm}"</p>
          <p style={{ marginTop: '0.25rem' }}>You can still use it as a custom reaction!</p>
        </div>
      )}
      ...
      {searchTerm.trim() && (
        <div className="border-t border-gray-200 pt-3">
          <form onSubmit={handleCustomEmojiSubmit} data-flex>
            <span style={{ fontSize: '0.875rem', color: 'var(--neutral-600)', flex: 1 }}>
      ...
      <button
        onClick={onClose}
        style={{ position: 'absolute', top: '0.25rem', right: '0.25rem', width: '1.5rem', height: '1.5rem', display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'var(--neutral-400)', cursor: 'pointer' }}
        title="Close"
      >
        ×
      </button>
