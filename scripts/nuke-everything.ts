...
const mediaDir = path.join(projectRoot, "data/media");
const thumbnailsDir = path.join(projectRoot, "data/thumbnails");
const dbPath = path.join(projectRoot, "data/db/media.db");
const syncTempDb = path.join(projectRoot, "data/.sync_temp_db.db");

const S3_PREFIX = "backup";

function s3Configured(): boolean {
  return ["AWS_ACCESS_KEY_ID", "AWS_SECRET_ACCESS_KEY", "AWS_REGION", "S3_BUCKET"].every(
...
async function main(): Promise<void> {
  console.log("\n\x1b[1;31m══════════════════════════════════════════════════════════════\x1b[0m");
  console.log("\x1b[1;31m  NUCLEAR RESET — this will PERMANENTLY destroy local and cloud data\x1b[0m");
  console.log("\x1b[1;31m══════════════════════════════════════════════════════════════\x1b[0m\n");

  const mediaStats = await dirStats(mediaDir);
  const thumbStats = await dirStats(thumbnailsDir);
  const syncTempExists = existsSync(syncTempDb);
  ...
  if (syncTempExists) {
    console.log(`  Sync temp DB          (${syncTempDb})`);
    console.log(`    → 1 file`);
  }
  ...
  if (!dbExists) {
    ...
    console.log(`    → ALL ROWS in ${Object.keys(dbRowCounts).length} table(s) (${totalRows} total rows):`);
  ...
  if (s3Configured()) {
    ...
    console.log(`    → ${s3Keys.length} object(s), ${formatBytes(s3Bytes)}`);
  } else {
  ...
  const answer = await ask(`Type the confirmation code \x1b[1m${code}\x1b[0m to proceed (or anything else to abort): `);
  ...
  if (syncTempExists) {
    await unlink(syncTempDb).catch(() => {});
    console.log("  Removed sync temp DB.");
  }

  if (s3Configured() && s3Keys.length > 0) {
    console.log(`\nDeleting ${s3Keys.length} S3 object(s)…`);
    const client = new S3Client({
      region: process.env.AWS_REGION!,
      credentials: {
        accessKeyId: process.env.AWS_ACCESS_KEY_ID!,
    ...
    await deleteS3Objects(client, process.env.S3_BUCKET!, s3Keys);
