...
**Chosen option:** **Subdomain split (option 3)** with **API at `api.lissner.io`** and **CORS allowing `https://lissner.io` and `https://*.lissner.io`** (host equals apex or ends with **`.lissner.io`** under HTTPS in production; see **`server/src/lib/lissner-cors.ts`**).
